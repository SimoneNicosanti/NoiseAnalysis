...
class NoiseAnalyzer:
    def __init__(
        ...
        self.model_quantizer: OnnxModelQuantizer = OnnxModelQuantizer(
            model_path, self.calib_set, providers
        )
        self.original_results = self._compute_model_results(
            self.model_quantizer.get_loaded_model(), self.eval_set
        )
        pass

    def _compute_model_results(
        self, onnx_model: onnx.ModelProto, dataset: list[np.ndarray]
    ):
        model_results = []
    ...
    def _compute_noise_list(
        self, quantized_model_results: list, noise_function: NoiseFunction
    ):
        noises = []
    ...
    def compute_avg_noise(
        ...
        noise_function: NoiseFunction = None,
    ):
