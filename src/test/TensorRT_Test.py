...
def run_inference(model_name: str, batch_size: int, provider: str):
    ...
    for i in range(timed_runs):
        ...
        time_ns = end - start
        times[i] = time_ns
    print("Model: {}, Provider: {}, Batch size: {}, Avg inference time: {} s".format(model_name, provider, batch_size, np.mean(times) / 1e9))

        


def main():
    parser = argparse.ArgumentParser(description="TensorRT / CUDA Quantization and Inference Test")
    parser.add_argument(
        "--model_name",
        type=str,
        required=True,
...
def hook_configure_ort():
    ...
    def on_ort_utils_import(ort_utils):
        ...
        original_configure_ort = getattr(new_ort_utils, 'configure_ort')
        ...
        setattr(new_ort_utils, 'configure_ort', patched_configure_ort)
